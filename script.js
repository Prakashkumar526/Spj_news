...
const add_entertainmentnews = (data) => { // Corrected function name
    let html = ``
    let title = ''
    data.forEach((element) => {
        if (element.title.length < 100) {
    ...
    entertainmentnews.innerHTML = html; // Corrected ID selector
}

fetchData('entertainment', 5).then(add_entertainmentnews) // Corrected function call